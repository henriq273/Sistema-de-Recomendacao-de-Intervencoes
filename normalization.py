...
def _dataset_filter(dataset_name: str) -> dict:
    """Áudio usa source.dataset; vídeo/imagem usam sourceMeta.dataset (confirmado por
    exemplos reais, ver data_source.normalize_audio_doc). Consultar por qualquer um
    dos dois evita que a auditoria fique silenciosamente vazia para datasets de áudio
    (ex.: DEAM, MEDITATION_LOCAL) por checar só o caminho de vídeo/imagem."""
    return {"$or": [{"sourceMeta.dataset": dataset_name}, {"source.dataset": dataset_name}]}


def audit_identity_dataset(collection, dataset_name: str) -> list:
    """
    ...
    fora_da_faixa = []
    for doc in collection.find(_dataset_filter(dataset_name)):  # leitura apenas
...
def audit_dataset(collection, dataset_name: str, tolerance: float = sysrec.NORMALIZATION_TOLERANCE) -> list:
    ...
    if ref["scale"] == "IDENTITY":
        fora_da_faixa = audit_identity_dataset(collection, dataset_name)
...
def check_zero_variance(collection) -> None:
    ...
    for dataset_name in sysrec.NORMALIZATION_REFERENCE:
        valores = [
            ...
            for doc in collection.find(_dataset_filter(dataset_name))
        ]
...
if __name__ == "__main__":
    ...
    raw_catalog, _ = data_source.build_raw_catalog()
