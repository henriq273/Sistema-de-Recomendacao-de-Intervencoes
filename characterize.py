...
def run_full_characterization() -> pd.DataFrame:
    ...
    if len(df) == 0:
        raise RuntimeError(
            "Catálogo vazio -- se SAFETY_CATEGORY_ALLOWLIST_ENABLED=True, confirme se "
            "APPROVED_CATEGORIES já foi populada (ver safety.explore_taxonomy); caso "
            "contrário, confirme se os exports em dbs/ (ou a conexão Mongo) têm itens "
            "com Valência/Arousal resolvíveis."
        )
